...
<style scoped>
.trajectory-panel { position: absolute; z-index: 4; right: clamp(1.5rem, 6vw, 6rem); bottom: clamp(5.6rem, 10vh, 7.5rem); display: flex; width: min(22rem, 28vw); max-height: calc(100% - clamp(6.6rem, 12vh, 8.5rem)); min-width: 0; flex-direction: column; padding: 1.15rem 1.3rem; border: 1px solid rgb(224 242 248 / 13%); background: linear-gradient(135deg, rgb(13 21 29 / 76%), rgb(5 8 13 / 50%)); box-shadow: 0 1.3rem 4rem rgb(0 0 0 / 18%); backdrop-filter: blur(14px); }
...
.trajectory-panel__description { min-height: 0; margin: 0; padding-right: 0.35rem; color: rgb(235 242 245 / 68%); font-size: 0.72rem; line-height: 1.65; overflow-wrap: anywhere; overflow-y: auto; scrollbar-color: rgb(155 222 248 / 32%) transparent; scrollbar-width: thin; white-space: pre-line; }
...
@media (max-width: 700px) { .trajectory-panel { position: relative; right: auto; bottom: auto; display: block; width: 100%; max-height: none; min-height: 8.5rem; padding: 0.9rem 1rem; } .trajectory-panel__description { padding-right: 0; overflow-y: visible; } .trajectory-panel__controls { display: grid; grid-template-columns: 1fr 1fr; } .trajectory-panel__controls .hud-control { min-height: 2.75rem; padding-inline: 0.5rem; letter-spacing: 0.1em; } .trajectory-panel__controls .hud-control:only-child { grid-column: 1 / -1; justify-self: end; } }
</style>
